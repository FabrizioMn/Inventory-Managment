...
class ProductoService:
    ...
    def crear_producto(producto: ProductoCreate):
        ...
        if sku_existente.data:
            raise HTTPException(
                ...
                detail=f"El SKU '{producto.sku}' ya está registrado en el sistema"
            )
        
        if producto.categoria_id:
            cat_existente = supabase.table("categoria").select("id_categoria").eq("id_categoria", producto.categoria_id).execute()
            if not cat_existente.data:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"La categoría con ID {producto.categoria_id} no existe"
                )
    ...
    def obtener_producto_por_sku(sku: str):
        ...
        if not response.data:
            raise HTTPException(status_code=404, detail=f"No se encontró ningún producto con el SKU {sku}")
        return response.data[0]